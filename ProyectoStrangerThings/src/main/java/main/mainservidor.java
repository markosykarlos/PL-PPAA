...
public class Mainservidor {
    ...
    public static void main(String[] args) {
        try{
            ...
            // Le pasamos los monitores a la ventana
            ...
            Demogorgon alpha = new Demogorgon(0, sotano, bosque, laboratorio, centrocomercial, 
                                              alcantarillado, colmena, estadoGlobal, sangre);
            alpha.start();

            // Al generador de niños ya no le instanciamos los portales dentro, se los pasamos hechos
           Creadorninos generadorNinos = new Creadorninos(sotano, bosque, laboratorio, centrocomercial, 
                                                          alcantarillado, portalBosque, portalLab, portalCC, 
                                                          portalAlc, estadoGlobal, sangre, log, callePrincipal, radioWSQK);
            generadorNinos.start();

                System.out.println("Iniciando registro RMI");

                LocateRegistry.createRegistry(1099);
                HawkinsRemoteImpl remoteImpl = new HawkinsRemoteImpl(ventana, estadoGlobal, colmena, bosque, laboratorio, 
                                                                    centrocomercial, alcantarillado, 
                                                                    portalBosque, portalLab, portalCC, portalAlc);
                Naming.rebind("//localhost/HawkinsServer", remoteImpl);
